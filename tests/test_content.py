...
def main_content_tester(
        user: Model,
        mixer: Mixer,
        post_model, user_client: Client,
        another_user_client: Client,
        unlogged_client: Client,
) -> MainPostContentTester:
    url_repr = UrlRepr("/", "/")
    return MainPostContentTester(
        mixer, post_model, PostModelAdapter, user_client, url_repr
    )
...
def category_content_tester(
    ...
    return CategoryPostContentTester(
        mixer, post_model, PostModelAdapter, user_client
    )
...
class TestContent:
    ...
    def test_unpublished_category(
            self, posts_with_unpublished_category
    ):
        profile_response = self.profile_tester.user_client_testget()
        context_posts = profile_response.context.get(
            self.profile_tester.items_key
    ...
    def test_pagination(
        self, many_posts_with_published_locations
    ):
        ...
        for (
            ...
            pagination_err_msg,
                ...
                (
                    ...
                    (
                        "Убедитесь, что публикации передаются"
                        " в контекст страницы"
                        " профиля автора отсортированными по"
                        " времени их"
                        " публикации, «от новых к старым»."
                    ),
                    (
                        "Убедитесь, что на странице профиля "
                        "автора работает"
                        " пагинация."
                    ),
                ),
                (
                    ...
                    (
                        "Убедитесь, что публикации передаются в контекст "
                        "страницы профиля автора отсортированными по "
                        "времени их публикации, «от новых к старым»."
                    ),
                    (
                ...
                (
                    ...
                    (
                        "Убедитесь, что публикации передаются в контекст "
                        "главной страницы отсортированными по времени их "
                        "публикации, «от новых к старым»."
                    ),
                    "Убедитесь, что на главной странице "
                    "пагинация работает в соответствии с заданием .",
