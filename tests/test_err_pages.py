    ...
    for status, fname in err_pages_vs_file_names.items():
        try:
            ...
            raise AssertionError(
                ...
                'является строкой (str) или объектом, '
                'соответствующим path-like интерфейсу '
                '(например, экземпляром pathlib.Path). '
                f'При операции конкатенации settings.TEMPLATES_DIR '
                f'/ "pages", возникла ошибка: {e}'
            )
