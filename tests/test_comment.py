...
@pytest.mark.usefixtures("comment_model", "comment_model_adapter")
...
class TestCommentModelAttrs(_TestModelAttrs):
    ...
    def _set_model(self, comment_model, comment_model_adapter):
        self._model = comment_model_adapter(comment_model)
...
def test_comment_created_at(comment):
    now = timezone.now()
...
def create_comment_creation_forms(
    ...
    forms_to_create = creation_tester.init_create_item_forms(
        form,
        Model=comment_model,
        ModelAdapter=comment_model_adapter,
        forms_unadapted_data=forms_data,
    )

    try:
        creation_tester.test_unlogged_cannot_create(
            form=forms_to_create[0], qs=comment_model.objects.all()
        )
...
def test_comment(
    ...
    comment_adapter = comment_model_adapter(created_items[0])
    ...
    created_item_adapters = [comment_model_adapter(i) for i in created_items]
    ...
    item_to_edit_adapter = comment_model_adapter(item_to_edit)
    ...
    _test_edit(
        edit_url,
        comment_model_adapter,
        item_to_edit,
        edit_form_tester=EditCommentFormTester,
        user_client=user_client,
