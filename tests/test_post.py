...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("pub_date", DateTimeField, {
            ...
            "Проверьте, что в модели Post в "
            "атрибуте pub_date параметр "
            "`auto_now_add` не установлен "
            "или имеет значение `False`. "
            "В ином случае станет невозможно "
            "публиковать посты "
...
def test_pos1(
    ...
    item_to_delete_adapter = PostModelAdapter(
        comment_model_adapter(comment_to_a_post).post
    )
...
def _test_create_items(
    ...
    response_on_created, created_items = creation_tester.test_create_several(
        forms=forms_to_create[1:], qs=post_model.objects.all()
    )
    content = response_on_created.content.decode(encoding="utf8")
    redirected_to_profile = any(
        [f'/profile/{post_adapter(created_items[0]).author.username}' in x[0]
...
def _test_edit_post(
    ...
    comment_adapter = comment_model_adapter(comment_to_a_post)
    ...
    edit_response = _test_edit(
        ...
        PostModelAdapter,
        item_to_edit,
        edit_form_tester=EditPostFormTester,
        user_client=user_client,
